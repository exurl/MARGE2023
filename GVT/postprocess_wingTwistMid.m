% Title: Postprocess GVT Data (v1.1) --> for mid-wing twist
% Author: Anthony Su
% Date: 2023-05-20, v1.1 update 2023-06-23,
%       mid-wing twist modification 2023-06-28
...
dt = 4; % truncate at dt seconds after impact
w = [1,200]; % czt_FRF bandwidth
window = 2; % czt_FTF window size, seconds
...
for idx = 1:length(dir_files)
    ...
    if(contains(f.name,'TIME_impact_') && contains(f.name,'sens_2214z') && endsWith(f.name,'.mat'))
...
%% DIFFERNCE SENSORS TO SENSE TWIST
for idx_file = 1:N_files
    for idx_test = 1:N_tests(idx_file)
        temp = data{idx_file,idx_test}{:,1};
        data{idx_file,idx_test}{:,1} = temp - data{idx_file,idx_test}{:,2};
        data{idx_file,idx_test}{:,2} = data{idx_file,idx_test}{:,2} - temp;
    end
end
...
for idx_file = 1:N_files
    for idx_test = 1:N_tests(idx_file)
        ...
        idx1 = idx_max-0.1*sampling_rate(idx_file); % retain 0.01 seconds before impact
        if(idx1<0)
            idx1 = 1;
        end
...
for idx_file = 1:N_files
    ...
    freq = f_avg{idx_file};
    mag_H1 = abs(FRF_H1{idx_file}); % NOT squared!
    mag_H2 = abs(FRF_H2{idx_file}); % NOT squared!
    coh = coh_avg{idx_file};
