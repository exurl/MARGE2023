...
dt = 50; % truncate at dt seconds after impact
w = [0.1,10]; % czt_FRF bandwidth
window = 20; % czt_FRF window size, seconds
...
for idx = 1:length(dir_files)
    ...
    if(contains(f.name,'TIME_impact_') && contains(f.name,'PITCHFREEGVT1'))
...
for idx_file = 1:N_files
    ...
    for idx_sensor = 1:N_sensors(idx_file)
        ...
        % PITCHFREEGVT1 ONLY
        filename = [filename,'_PITCHFREEGVT1'];
