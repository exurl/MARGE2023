...
sensor_locations = {'wingz_long'};
files = {};
for idx_impact = 1:length(impact_locations)
    for idx_sensor = 1:length(sensor_locations) % both input and output sensors
        files{end+1} = ['impact_',impact_locations{idx_impact},'_sens_',sensor_locations{idx_sensor},'_2023-05-26','.mat'];
    end
end
...
dt = 20; % retain dt seconds after impact
...
w = [0.25,10]; % <-- FREQUENCY BANDWIDTH FOR CZT FRF
...
for idx_file = 3:N_files % ignore 2014z and 2020z hammer tests
    for idx_sensor = 2:N_sensors(idx_file) % ignore 2014z sensor
