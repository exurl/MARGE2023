% Title: Postprocess Window Study
% Author: Anthony Su
% Date: 2023-05-25
...
%% CZT_FRF WINDOW LENGTHS TO TEST
N_window = linspace(90000,330000,5);
line_widths = [1.4,1.2,1.0,0.8,0.6];
...
for idx_file = 1:N_files
    ...
    for idx_sensor = 1:N_sensors(idx_file) % output sensors only
        ...
        % compute
        w = [1.2,100];
        for idx_window = 1:length(N_window)
            [FRF_avg{idx_window,idx_sensor,idx_file},...
                f_avg{idx_window,idx_sensor,idx_file},...
                coh_avg{idx_window,idx_sensor,idx_file}] = ...
                czt_FRF(u,y,N_window(idx_window),w,'off',sampling_rate(idx_file));
        end
    end
end
...
for idx_file = 1:N_files
    for idx_sensor = 1:N_sensors(idx_file)
        ...
        for idx_window = 1:length(N_window)
            nexttile(tl,1)
            plot(f_avg_{idx_window},mag2db(abs(FRF_avg_{idx_window})),'LineWidth',line_widths(idx_window))
            % plot phase
            nexttile(tl,2)
            plot(f_avg_{idx_window},rad2deg(phase(FRF_avg_{idx_window})),'LineWidth',line_widths(idx_window))
            % plot coherence
            nexttile(tl,3)
            plot(f_avg_{idx_window},coh_avg_{idx_window},'LineWidth',line_widths(idx_window))

            line_labels = [line_labels,{['window = ',num2str(N_window(idx_window)/51200),' s']}];
        end
