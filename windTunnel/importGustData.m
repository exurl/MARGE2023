...
files = ["2023 summer/20230816/DG4_R1/output.mat",...
    "2023 summer/20230816/DG4_R2/output.mat",...
    "2023 summer/20230816/DG4_R3/output.mat"];
    % q=207 Pa --> ~u=19 m/s
...
for idxFile = 1:length(files)
    ...
    % % truncate before input starts, 5s after input ends
    ...
    % truncate before input starts, after most active signal drops to 50% of max magnitude
    % inputMask = find(sum(u~=0,2)); % indices with nonzero input
    % idx1 = inputMask(1)-1; % start 1 datapoint before input
    % [~,idxActive] = max(var(y./max(y)));
    % yActive = y(:,idxActive);
    % activeIdxs = find(abs(yActive)>0.5*max(abs(yActive)));
    % idx2 = activeIdxs(end);
